...
websocket_t * new_websocket ( ip_addr_t * ip, u16_t port ){
	...
	websocket_t * websocket = ( websocket_t * ) heap_caps_malloc( sizeof( websocket_t ), MALLOC_CAP_8BIT );
...
err_t websocket_received(void *arg, struct tcp_pcb *tpcb, struct pbuf *p, err_t err){
	...
	case websocket_status_closed:
		...
		if ( websocket_get_key( p->payload, p->len, websocket ) ){
			char * buffer = ( char * ) heap_caps_malloc( 256, MALLOC_CAP_8BIT );
...
websocket_frame_t * websocket_decode_frame ( char * frame_ptr, u16_t len ){
	...
	websocket_frame_t * frame = ( websocket_frame_t * ) heap_caps_malloc( sizeof ( websocket_frame_t ), MALLOC_CAP_8BIT );
	...
	if ( data_len < 126 ) {
		...
		frame->data = ( u8_t * ) heap_caps_malloc ( frame->length + 1 , MALLOC_CAP_8BIT );
...
u8_t * websocket_encode_frame( websocket_opcode_t opcode, u8_t * data, u16_t len ){
	...
	if ( len < 126 ){
		u8_t * data_ptr = ( u8_t * ) heap_caps_malloc( len + 3, MALLOC_CAP_8BIT );
		...
		u8_t * data_ptr = ( u8_t * ) heap_caps_malloc( len + 5, MALLOC_CAP_8BIT );
...
err_t websocket_send_api_call(struct tcpip_api_call* call){
	...
	if ( send_api_data->data ) free( send_api_data->data );
